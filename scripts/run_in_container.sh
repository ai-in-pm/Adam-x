...
# Remove any existing container named 'adam-x'.
docker rm -f adam-x 2>/dev/null || true

# Run the container with the specified directory mounted at the same path inside the container.
docker run --name adam-x -d \
  -e OPENAI_API_KEY \
  --cap-add=NET_ADMIN \
  --cap-add=NET_RAW \
...
docker exec adam-x bash -c "sudo /usr/local/bin/init_firewall.sh"
...
docker exec -it adam-x bash -c "cd \"/app$WORK_DIR\" && adam-x --full-auto ${quoted_args}"
