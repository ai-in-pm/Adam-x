...
export const CONFIG_DIR = join(homedir(), ".adam-x");
...
export const GEMINI_1_5_API_KEY = process.env["GEMINI_1_5_API_KEY"] || "";
export const MISTRAL_7B_API_KEY = process.env["MISTRAL_7B_API_KEY"] || "";
export const GROK_1_API_KEY = process.env["GROK_1_API_KEY"] || "";
export const PALM_2_API_KEY = process.env["PALM_2_API_KEY"] || "";
export const FALCON_180B_API_KEY = process.env["FALCON_180B_API_KEY"] || "";
export const STABLE_LM_2_API_KEY = process.env["STABLE_LM_2_API_KEY"] || "";
export const LLAMA_3_1_API_KEY = process.env["LLAMA_3_1_API_KEY"] || "";
export const MIXTRAL_8X22B_API_KEY = process.env["MIXTRAL_8X22B_API_KEY"] || "";
export const INFLECTION_2_5_API_KEY = process.env["INFLECTION_2_5_API_KEY"] || "";
export const JAMBA_API_KEY = process.env["JAMBA_API_KEY"] || "";
export const COMMAND_R_API_KEY = process.env["COMMAND_R_API_KEY"] || "";
export const GEMMA_API_KEY = process.env["GEMMA_API_KEY"] || "";
export const PHI_3_API_KEY = process.env["PHI_3_API_KEY"] || "";
export const XGEN_7B_API_KEY = process.env["XGEN_7B_API_KEY"] || "";
export const DBRX_API_KEY = process.env["DBRX_API_KEY"] || "";
export const PYTHIA_API_KEY = process.env["PYTHIA_API_KEY"] || "";
export const SORA_API_KEY = process.env["SORA_API_KEY"] || "";
export const ALPACA_7B_API_KEY = process.env["ALPACA_7B_API_KEY"] || "";
export const NEMOTRON_4_API_KEY = process.env["NEMOTRON_4_API_KEY"] || "";
...
export function setApiKey(apiKey: string, provider: string = "openai"): void {
  switch (provider) {
    ...
    case "anthropic":
      ...
      break;
    case "google":
      process.env["GEMINI_1_5_API_KEY"] = apiKey;
      break;
    case "mistral":
      process.env["MISTRAL_7B_API_KEY"] = apiKey;
      break;
    default:
...
// ---------------------------------------------------------------------------
// Project doc support (adam-x.md)
// ---------------------------------------------------------------------------

export const PROJECT_DOC_MAX_BYTES = 32 * 1024; // 32 kB

const PROJECT_DOC_FILENAMES = ["adam-x.md", ".adam-x.md", "ADAM-X.md"];
...
/**
 * Load the project documentation markdown (adam-x.md) if present. If the file
...
export function loadProjectDoc(cwd: string, explicitPath?: string): string {
  ...
  if (explicitPath) {
    ...
    if (!existsSync(filepath)) {
      ...
      console.warn(`adam-x: project doc not found at ${filepath}`);
      filepath = null;
    }
  } else {
  ...
  try {
    ...
    if (buf.byteLength > PROJECT_DOC_MAX_BYTES) {
      // eslint-disable-next-line no-console
      console.warn(
        `adam-x: project doc '${filepath}' exceeds ${PROJECT_DOC_MAX_BYTES} bytes – truncating.`,
      );
    }
...
export const loadConfig = (
  ...
  const shouldLoadProjectDoc =
    ...
    process.env["ADAM_X_DISABLE_PROJECT_DOC"] !== "1";
  ...
  if (shouldLoadProjectDoc) {
    ...
    if (projectDocPath) {
      if (isLoggingEnabled()) {
        log(
          `[adam-x] Loaded project doc from ${projectDocPath} (${projectDoc.length} bytes)`,
        );
      }
    } else {
      if (isLoggingEnabled()) {
        log(`[adam-x] No project doc found in ${cwd}`);
      }
    }
  }
  ...
  try {
    if (!existsSync(actualConfigPath)) {
      ...
      // pinning users to an old default after upgrading Adam-X.
      const ext = extname(actualConfigPath).toLowerCase();
      if (ext === ".yaml" || ext === ".yml") {
    ...
    // block the CLI from starting.  A future explicit `adam-x config` command
