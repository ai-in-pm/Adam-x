...
// Call this early so `tail -F "$TMPDIR/oai-adam-x/adam-x-cli-latest.log"` works
...
const cli = meow(
  `
  Usage
    $ adam-x [options] <prompt>
    $ adam-x <command> [subcommand] [options]

  Commands
    snippet, snippets, snip    Generate and manage code snippets
                               Run 'adam-x snippet help' for more information

    llm, model, provider      Manage and switch between different LLM providers
                               Run 'adam-x llm help' for more information

    whitelist                Manage whitelisted commands for network-enabled execution
                               Run 'adam-x whitelist help' for more information
  ...
  Options
    ...
    --no-project-doc           Do not automatically include the repository's 'adam-x.md'
  ...
  Examples
    $ adam-x "Write and run a python program that prints ASCII art"
    $ adam-x -q "fix build issues"
    $ adam-x snippet generate javascript "Express.js REST API endpoint"
...
`,
  {
    ...
    flags: {
      ...
      approvalMode: {
        ...
        aliases: ["a"],
        description:
          "Determine the approval mode for Adam-X (default: suggest) Values: suggest, auto-edit, full-auto",
      },
      noProjectDoc: {
        type: "boolean",
        description: "Disable automatic inclusion of project‑level adam-x.md",
      },
      projectDoc: {
        type: "string",
...
if (cli.input.length > 0 && [
  ...
  'llm', 'model', 'provider',
  'whitelist'
].includes(cli.input[0])) {
  await handleCommand(cli.input[0], cli.input.slice(1));
  onExit();
  process.exit(0);
}
...
if (quietMode) {
  process.env["ADAM_X_QUIET_MODE"] = "1";
  if (!prompt || prompt.trim() === "") {
    // eslint-disable-next-line no-console
    console.error(
      'Quiet mode requires a prompt string, e.g.,: adam-x -q "Fix bug #123 in the foobar project"',
    );
    process.exit(1);
  }
  await runQuietMode({
