...
export function execWithSeatbelt(
  ...
  if (writableRoots.length > 0) {
    // Add `~/.adam-x` to the list of writable roots
  ...
  // Add network access policy if allowed
  const networkPolicy = allowNetwork ? NETWORK_ACCESS_POLICY : "";

  const fullPolicy = READ_ONLY_SEATBELT_POLICY + scopedWritePolicy + networkPolicy;
  log(
    `Running seatbelt with policy: ${fullPolicy} and ${
      policyTemplateParams.length
    } template params: ${policyTemplateParams.join(", ")}, allowNetwork: ${allowNetwork}`,
  );
...
// Policy to allow network access
const NETWORK_ACCESS_POLICY = `
