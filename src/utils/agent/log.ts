...
/**
 * Creates a .log file for this session, but also symlinks adam-x-cli-latest.log
 * to the current log file so you can reliably run:
 *
 * - Mac/Windows: `tail -F "$TMPDIR/oai-adam-x/adam-x-cli-latest.log"`
 * - Linux: `tail -F ~/.local/oai-adam-x/adam-x-cli-latest.log`
 */
export function initLogger(): Logger {
  ...
  // it there. On Linux, use ~/.local/oai-adam-x so logs are not world-readable.
  const logDir =
    isMac || isWin
      ? path.join(os.tmpdir(), "oai-adam-x")
      : path.join(os.homedir(), ".local", "oai-adam-x");
  fsSync.mkdirSync(logDir, { recursive: true });
  const logFile = path.join(logDir, `adam-x-cli-${now()}.log`);
  // Write the empty string so the file exists and can be tail'd.
  fsSync.writeFileSync(logFile, "");

  // Symlink to adam-x-cli-latest.log on UNIX because Windows is funny about
  // symlinks.
  if (!isWin) {
    const latestLink = path.join(logDir, "adam-x-cli-latest.log");
    try {
