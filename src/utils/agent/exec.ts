...
export function exec(
  ...
  switch (sandbox) {
    case SandboxType.MACOS_SEATBELT:
      execForSandbox = execWithSeatbelt;
      break;
    case SandboxType.MACOS_SEATBELT_NETWORK:
      // Use the seatbelt sandbox but allow network access
      execForSandbox = (cmd, opts, writableRoots, abortSignal) =>
        execWithSeatbelt(cmd, opts, writableRoots, abortSignal, true);
      break;
    case SandboxType.CONTAINER_NETWORK:
      // Use raw exec but with network access in container
      execForSandbox = rawExec;
      // TODO: Implement container network access
      break;
    default:
      execForSandbox = rawExec;
      break;
