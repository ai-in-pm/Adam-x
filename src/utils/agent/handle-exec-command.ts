...
export async function handleExecCommand(
  ...
  let allowNetwork: boolean = false; // Default to no network access
  ...
  switch (safety.type) {
    ...
    case "auto-approve": {
      ...
      // If the command is whitelisted for network access, set allowNetwork to true
  ...
  if (
    ...
    if (review != null) {
      ...
      // sandbox.
      const summary = await execCommand(args, applyPatch, false, abortSignal, allowNetwork);
...
async function execCommand(
  ...
  const execResult =
    applyPatchCommand != null
      ...
      : await exec(execInput, await getSandbox(runInSandbox, allowNetwork), abortSignal);
...
async function getSandbox(runInSandbox: boolean, allowNetwork: boolean = false): Promise<SandboxType> {
  if (isLoggingEnabled()) {
    log(`Getting sandbox: runInSandbox=${runInSandbox}, allowNetwork=${allowNetwork}`);
  }

  if (runInSandbox) {
    if (process.platform === "darwin") {
      return allowNetwork ? SandboxType.MACOS_SEATBELT_NETWORK : SandboxType.MACOS_SEATBELT;
    } else if (await isInContainer()) {
      return allowNetwork ? SandboxType.CONTAINER_NETWORK : SandboxType.NONE;
    }
