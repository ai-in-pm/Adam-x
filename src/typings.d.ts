...
// cover only the APIs that the Adam-X codebase relies on. If full type
...
declare module "shell-quote" {
  ...
  // type definitions. We recreate the few that Adam-X imports so that the
   ...
   * plain string is sufficient for type‑checking the Adam-X codebase while
   ...
   * redirection …) which we deliberately omit here because Adam-X never
   * inspects them.
   */
