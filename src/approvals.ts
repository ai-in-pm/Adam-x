...
export type SafetyAssessment = {
  ...
  | {
      ...
      /**
       * If true, the command is allowed to access the network.
       * This should only be true for whitelisted commands.
       */
...
export function canAutoApprove(
  ...
  // Check if the command is whitelisted for network access
  const whitelisted = isWhitelisted(command);
  if (whitelisted) {
    return {
      type: "auto-approve",
      reason: `Whitelisted command: ${whitelisted.reason}`,
      group: "Whitelisted",
      runInSandbox: false,
      allowNetwork: whitelisted.allowNetwork,
    };
  }
